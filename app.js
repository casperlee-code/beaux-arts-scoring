...
const LANDMARKS = [
  ...
  },
  {
    ...
    story: "Prada 青山旗艦店是瑞士建築大師 Herzog & de Meuron 的神來之筆。整棟大樓外觀如同顆琢磨過的鑽石水晶，完全由綠色的菱形網格玻璃幕牆拼裝而成。其中，玻璃塊面分為凹面、凸面及平面三種。行人在外面走過，或是店內顧客走動，都會看到窗外東京街景與人影被反射、放大與變形，打破了「牆」的內外阻隔，是一棟徹底探索視覺與知覺的頂尖時尚地標。它是東京最著名的「眼球磁鐵」，展現了晶瑩剔透、輕盈流動的幾何張力。",
    highlights: [
...
// ==========================================
// LOCAL GEOCODING DICTIONARY (Fallback system)
// ==========================================
const LOCAL_GEOCODE_DICT = {
  "根津美術館": { coords: [35.6622, 139.7180], addr: "港區 南青山 6-5-1" },
  "國立新美術館": { coords: [35.6653, 139.7263], addr: "港區 六本木 7-22-2" },
  "森美術館": { coords: [35.6605, 139.7291], addr: "港區 六本木 6-10-1 (森大樓 53F)" },
  "東京車站": { coords: [35.6812, 139.7671], addr: "千代田區 丸之內 1 丁目" },
  "東京站": { coords: [35.6812, 139.7671], addr: "千代田區 丸之內 1 丁目" },
  "東京鐵塔": { coords: [35.6586, 139.7454], addr: "港區 芝公園 4-2-8" },
  "表參道之丘": { coords: [35.6672, 139.7093], addr: "澀谷區 神宮前 4-12-10" },
  "21_21 DESIGN SIGHT": { coords: [35.6660, 139.7314], addr: "港區 赤坂 9-7-6 (東京防城)" },
  "21 21 DESIGN SIGHT": { coords: [35.6660, 139.7314], addr: "港區 赤坂 9-7-6 (東京防城)" },
  "代官山 T-SITE": { coords: [35.6491, 139.7001], addr: "澀谷區 猿樂町 17-5" },
  "代官山蔦屋書店": { coords: [35.6491, 139.7001], addr: "澀谷區 猿樂町 17-5" },
  "銀座 SIX": { coords: [35.6696, 139.7640], addr: "中央區 銀座 6-10-1" },
  "GINZA SIX": { coords: [35.6696, 139.7640], addr: "中央區 銀座 6-10-1" },
  "國立代代木競技場": { coords: [35.6679, 139.6997], addr: "澀谷區 神南 2-1-1" },
  "代代木競技場": { coords: [35.6679, 139.6997], addr: "澀谷區 神南 2-1-1" },
  "東京晴空塔": { coords: [35.7101, 139.8107], addr: "墨田區 押上 1-1-2" },
  "晴空塔": { coords: [35.7101, 139.8107], addr: "墨田區 押上 1-1-2" },
  "淺草文化觀光中心": { coords: [35.7110, 139.7963], addr: "台東區 淺草 2-18-9" },
  "淺草觀光中心": { coords: [35.7110, 139.7963], addr: "台東區 淺草 2-18-9" },
  "東京國際論壇": { coords: [35.6778, 139.7640], addr: "千代田區 丸之內 3-5-1" },
  "普拉達青山旗艦店": { coords: [35.6622, 139.7153], addr: "港區 南青山 5-2-6" },
  "Prada青山店": { coords: [35.6622, 139.7153], addr: "港區 南青山 5-2-6" },
  "明治神宮": { coords: [35.6763, 139.6993], addr: "澀谷區 代代木神園町 1-1" },
  "微熱山丘": { coords: [35.6631, 139.7169], addr: "港區 南青山 3-10-20" },
  "微熱山丘南青山": { coords: [35.6631, 139.7169], addr: "港區 南青山 3-10-20" },
  "築地本願寺": { coords: [35.6661, 139.7722], addr: "中央區 築地 3-15-1" },
  "中銀膠囊大樓": { coords: [35.6682, 139.7628], addr: "中央區 銀座 8-16-10" },
  "中銀大樓": { coords: [35.6682, 139.7628], addr: "中央區 銀座 8-16-10" },
  "淺草寺": { coords: [35.7148, 139.7967], addr: "台東區 淺草 2-3-1" },
  "上野恩賜公園": { coords: [35.7154, 139.7740], addr: "台東區 上野公園" },
  "皇居": { coords: [35.6852, 139.7528], addr: "千代田區 千代田 1-1" },
  "澀谷十字路口": { coords: [35.6595, 139.7006], addr: "澀谷區 道玄坂 2 丁目" }
};
...
class TokyoArchApp {
  constructor() {
    ...
    // User Planned Custom spots state
    this.userPlannedSpots = JSON.parse(localStorage.getItem("userPlannedSpots")) || [];
    this.userMarkers = [];
    ...
    this.renderUserSpotsList(); // Display saved custom spots from localstorage
    this.renderUserMarkers();   // Display saved custom map markers
    this.initEvents();
    this.initSakura();
  }
  ...
  initElements() {
    ...
    // NEW Custom Itinerary Elements
    this.tabMasterLandmarks = document.getElementById("tab-master-landmarks");
    this.tabCustomItinerary = document.getElementById("tab-custom-itinerary");
    this.customItineraryPanel = document.getElementById("custom-itinerary-panel");
    this.userSpotsCount = document.getElementById("user-spots-count");
    this.userSpotsCountList = document.getElementById("user-spots-count-list");
    
    this.userPinsTextarea = document.getElementById("user-pins-textarea");
    this.importPinsBtn = document.getElementById("import-pins-btn");
    this.clearCustomPinsBtn = document.getElementById("clear-custom-pins-btn");
    this.importLoadingIndicator = document.getElementById("import-loading-indicator");
    this.userSpotsListContainer = document.getElementById("user-spots-list-container");
  }
  ...
  // Render Map Pin Markers and Bind Synchronization (Master Landmarks)
  ...
  // Render custom User planned pins
  renderUserMarkers() {
    this.userMarkers.forEach(m => this.map.removeLayer(m));
    this.userMarkers = [];

    if (this.userPlannedSpots.length === 0) return;

    this.userPlannedSpots.forEach(item => {
      // Distinct Emerald green pin
      const customIcon = L.divIcon({
        className: 'custom-div-icon',
        html: `<div class="marker-pin-wrapper user-pin" id="user-pin-${item.id}"></div>`,
        iconSize: [32, 32],
        iconAnchor: [16, 32],
        popupAnchor: [0, -30]
      });
      ...
      const popupContent = `
        <div class="map-popup-card">
          <h4>📍 ${item.name}</h4>
          <p style="font-size: 0.75rem; color: var(--color-text-muted); margin-bottom: 8px;">${item.addr}</p>
          <div class="popup-link" onclick="window.app.deleteUserSpot('${item.id}')" style="color: var(--color-accent)">
            <i class="fa-solid fa-trash-can"></i> <span>移除此景點</span>
          </div>
        </div>
      `;
      ...
      marker.on('click', () => {
        this.highlightUserSpotCard(item.id);
      });
  ...
  focusStepLandmark(title) {
    ...
    if (match) {
      ...
      setTimeout(() => {
        // Swap to Master Landmarks Tab
        this.switchExplorerTab("master");
  ...
  // ==========================================
  // NEW: CUSTOM PILGRIMAGE PIN INPUT FUNCTIONS
  // ==========================================

  // Switch right panel tabs
  switchExplorerTab(tabType) {
    if (tabType === "master") {
      this.tabMasterLandmarks.classList.add("active");
      this.tabCustomItinerary.classList.remove("active");
      this.cardsContainer.style.display = "flex";
      this.customItineraryPanel.style.display = "none";
    } else {
      this.tabCustomItinerary.classList.add("active");
      this.tabMasterLandmarks.classList.remove("active");
      this.cardsContainer.style.display = "none";
      this.customItineraryPanel.style.display = "flex";
    }
  }

  // Display Custom planned spots list
  renderUserSpotsList() {
    const count = this.userPlannedSpots.length;
    this.userSpotsCount.textContent = count;
    this.userSpotsCountList.textContent = count;

    this.userSpotsListContainer.innerHTML = "";

    if (count === 0) {
      this.userSpotsListContainer.innerHTML = `
        <div class="empty-explorer-state" style="padding: 30px 16px; border-style: dotted;">
          <i class="fa-solid fa-map-pin" style="font-size: 2rem;"></i>
          <h4 style="font-family: var(--font-serif); font-size: 1.05rem;">清單目前是空的</h4>
          <p style="font-size: 0.78rem;">請在上方框內輸入景點名稱以匯入地圖網站，建立您專屬的東京朝聖行程。</p>
        </div>
      `;
      return;
    }
    ...
    this.userPlannedSpots.forEach(item => {
      ...
      card.className = "user-spot-card";
      card.id = `user-card-${item.id}`;
      card.addEventListener("click", () => this.focusUserSpot(item.id));

      card.innerHTML = `
        <div class="user-spot-info">
          <span class="user-spot-name">${item.name}</span>
          <span class="user-spot-addr"><i class="fa-solid fa-location-dot"></i> ${item.addr}</span>
          <span class="user-spot-coords">${item.coords[0].toFixed(5)}, ${item.coords[1].toFixed(5)}</span>
        </div>
        <div class="user-spot-actions" onclick="event.stopPropagation()">
          <button class="user-spot-delete-btn" onclick="window.app.deleteUserSpot('${item.id}')" title="移除此景點">
            <i class="fa-solid fa-trash-can"></i>
          </button>
        </div>
      `;

      this.userSpotsListContainer.appendChild(card);
    });
  }

  // Focus User spot from planned list
  focusUserSpot(id) {
    document.querySelectorAll(".user-spot-card").forEach(c => c.classList.remove("active"));
    
    const cardEl = document.getElementById(`user-card-${id}`);
    if (cardEl) {
      cardEl.classList.add("active");
    }

    const spot = this.userPlannedSpots.find(s => s.id === id);
    if (spot) {
      this.map.setView(spot.coords, 14, { animate: true, duration: 0.8 });
      
      const marker = this.userMarkers.find(m => m.landmarkId === id);
      if (marker) {
        marker.openPopup();
        
        document.querySelectorAll(".marker-pin-wrapper").forEach(p => p.classList.remove("active"));
        const pinWrapper = document.getElementById(`user-pin-${id}`);
        if (pinWrapper) pinWrapper.classList.add("active");
      }
    }
  }

  // Highlight spot card inside Custom List when marker is clicked
  highlightUserSpotCard(id) {
    document.querySelectorAll(".user-spot-card").forEach(c => c.classList.remove("active"));
  ...
  // Delete planned spot
  deleteUserSpot(id) {
    this.userPlannedSpots = this.userPlannedSpots.filter(s => s.id !== id);
    localStorage.setItem("userPlannedSpots", JSON.stringify(this.userPlannedSpots));
    
    this.renderUserSpotsList();
    this.renderUserMarkers();
  }

  // Asynchronous geocoder client parsing
  async handleImportPins() {
    const rawText = this.userPinsTextarea.value.trim();
    if (!rawText) return;

    // Split by newlines or commas
    const lines = rawText.split(/[\n,;，；]+/);
    const names = lines
      .map(line => line.replace(/^\d+[\.\-\s、]+/, "").trim()) // clean numbers e.g. "1. " or "2、"
      .filter(name => name.length > 0);

    if (names.length === 0) return;

    this.importPinsBtn.disabled = true;
    this.importLoadingIndicator.style.display = "flex";
    ...
    for (let i = 0; i < names.length; i++) {
      const name = names[i];
      // Geocoding step with Nominatim fallback dictionary
      const result = await this.geocodeSpot(name);
      
      if (result) {
        // Prevent duplication
        if (!this.userPlannedSpots.some(s => s.name === result.name)) {
          const newSpot = {
          ...
          this.userPlannedSpots.push(newSpot);
          addedSpots.push(newSpot);
        }
      }

      // OSM Nominatim rate limit safety delay (600ms)
      if (i < names.length - 1) {
        await sleep(600);
      }
    }
    ...
    // Clear textbox & loader
    this.userPinsTextarea.value = "";
    this.importPinsBtn.disabled = false;
    this.importLoadingIndicator.style.display = "none";

    // Refresh UI
    this.renderUserSpotsList();
    this.renderUserMarkers();

    // Focus on first imported spot
    if (addedSpots.length > 0) {
      this.focusUserSpot(addedSpots[0].id);
    } else {
      alert("匯入完成。未發現新地標或地標名稱查無座標，請重新確認拼寫！");
    }
  }

  // Geocoding query method with Nominatim OSM geocoding API
  async geocodeSpot(name) {
    const cleanName = name.trim();
    
    // 1. Try local dictionary (Instant and accurate for top spots)
    for (const key in LOCAL_GEOCODE_DICT) {
      if (cleanName.includes(key) || key.includes(cleanName)) {
        return {
          name: key, // Use standard name
          coords: LOCAL_GEOCODE_DICT[key].coords,
          addr: LOCAL_GEOCODE_DICT[key].addr
        };
      }
    }

    // 2. Fetch OpenStreetMap Nominatim Geocoding API
    try {
      // Append "Tokyo" to query to narrow search scope to Tokyo region
      const searchUrl = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent('Tokyo ' + cleanName)}&accept-language=zh-TW`;
      
      const res = await fetch(searchUrl, {
        headers: {
          'Accept': 'application/json',
          'User-Agent': 'TokyoArchitectureTravelGuide/1.0'
        }
      });
      ...
      if (data && data.length > 0) {
        ...
        // Clean up OSM address format (display_name is usually long comma separated strings)
        const fullAddr = top.display_name;
        const parts = fullAddr.split(", ");
        
        // Pick a short subset to show district/ward
        let shortAddr = "東京地區";
        if (parts.length >= 3) {
          // Join first 2-3 fields depending on hierarchy
          shortAddr = parts.slice(0, 3).filter(p => !p.includes(cleanName)).join(", ");
        }
      ...
      console.error("OSM Geocoding failed for name: " + cleanName, err);
    }
  ...
  clearCustomSpots() {
    if (this.userPlannedSpots.length === 0) return;
    if (confirm("您確定要清空所有已規劃的專屬景點嗎？")) {
      this.userPlannedSpots = [];
      localStorage.removeItem("userPlannedSpots");
  ...
  initEvents() {
    this.themeToggleBtn.addEventListener("click", () => {
      ...
      this.updateMapTiles();
      this.renderLandmarks();
      this.renderUserMarkers(); // Redraw custom user markers in theme compatibility
    });
    ...
    this.filterTabsContainer.addEventListener("click", (e) => {
      ...
      // Auto toggle to Master Tab to display filter matches
      this.switchExplorerTab("master");
      this.renderLandmarks();
    });
    ...
    // Custom tab toggle listeners
    this.tabMasterLandmarks.addEventListener("click", () => this.switchExplorerTab("master"));
    this.tabCustomItinerary.addEventListener("click", () => this.switchExplorerTab("custom"));
    
    // Import custom pin buttons triggers
    this.importPinsBtn.addEventListener("click", () => this.handleImportPins());
    this.clearCustomPinsBtn.addEventListener("click", () => this.clearCustomSpots());
